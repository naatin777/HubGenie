...
type KeyAction = "enter" | "up" | "down" | "exit" | null;
...
async function readKey(): Promise<KeyAction> {
  ...
  if (bytes.length === 1 && bytes[0] === 106) return "down";
  if (bytes.length === 1 && bytes[0] === 107) return "up";
...
async function render<T>(
  ...
  const title = `${bold(`? ${message}`)} ${counter}\n`;
  output += title;
...
export async function selectPrompt<T>(options: SelectOptions<T>): Promise<T> {
